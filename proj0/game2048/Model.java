...
public class Model extends Observable {
    ...
    public boolean tilt(Side side) {
        ...
        for ( int looptimes = 0; looptimes < 3; looptimes += 1){
            for (int y = size() - 2; y >= 0; y -= 1) {
                for (int x = 0; x < size(); x += 1) {
                    if ( board.tile( x , y ) != null){
                        Tile t = board.tile( x , y );
                        if ( board.tile( x, y+1 ) == null){
                            board.move( x, y+1, t);
                            changed = true;
                        }
                    }
    ...
    public static boolean atLeastOneMoveExists(Board b) {
        ...
        for (int i=0; i < (b.size()-1); i += 1) {
            for (int n = 0; n < b.size(); n += 1) {
                if (b.tile(i,n).value()==b.tile(i+1,n).value()) {
                    return true;
                }
            }
        }
        for (int i=0; i < b.size(); i += 1) {
            for (int n = 0; n < (b.size()-1); n += 1) {
